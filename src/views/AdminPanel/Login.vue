...
<template>
    <div class="min-h-screen flex items-center justify-center bg-img">
      <div class="bg-white py-20 px-20 rounded-3xl shadow-lg max-w-xl w-full">
        <h2 class="text-4xl font-semibold text-center mb-2">Admin Login</h2>
  ...
  const handleLogin = () => {
    ...
    axios.post(`${ADMIN_API_URL}?type=login`, formData)
      .then((resp) => {
        ...
        if(resp.data.status) {
            ...
            localStorage.setItem('x-admin-login','true')
            //save user datato localstorage
            localStorage.setItem('x-admin', JSON.stringify(resp.data['admin']))
            speak('x-login',  true)
            //redirect to profile
